...
- (void)test_pluginTemplateLog_isNotNil {
	XCTAssertNotEqual(
			plugin_template_log, (os_log_t)NULL, @"plugin_template_log must be initialized before any test runs");
}
...
- (void)test_pluginTemplateLog_isValidOSLogObject {
	...
	XCTAssertNoThrow(os_log_debug(plugin_template_log, "PluginTemplateLoggerTests: sanity write"),
...
- (void)test_pluginTemplateLog_acceptsFormatArguments {
	XCTAssertNoThrow(
			{
				os_log_debug(plugin_template_log, "PluginTemplateLoggerTests: int=%d float=%f string=%{public}s", 42,
						3.14, "hello");
			},
...
- (void)test_pluginTemplateLog_acceptsInfoLevel {
	XCTAssertNoThrow(os_log_info(plugin_template_log, "PluginTemplateLoggerTests: info-level write"),
			@"Info-level writes must not crash");
}

- (void)test_pluginTemplateLog_acceptsErrorLevel {
	XCTAssertNoThrow(os_log_error(plugin_template_log, "PluginTemplateLoggerTests: error-level write"),
